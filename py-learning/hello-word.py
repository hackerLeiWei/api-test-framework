...
print(fullName)
print(f"name:{firstName.lstrip()} length: {firstName.lstrip().__len__()}"
      )  # 删除左侧空格
print(f"name:{lastName.rstrip()} length: {lastName.rstrip().__len__()}"
      )  # 删除右侧空格
print(
    f"name:{fullName.strip()} length: {fullName.strip().__len__()}")  # 删除左右空格
...
print(
    f"推导式 squars =[value**2 for value in range(1,9)]=:{squars}  hex:{hex(id(squars))}\n"
)
...
print(
    f"numbers == numbers3:{numbers == numbers3}  {numbers}-{hex(id(numbers))}  {numbers3}-{hex(id(numbers3))}\n"
)
print(
    f"hex(id(numbers)) == hex(id(numbers3)):{ {hex(id(numbers))} == {hex(id(numbers3))}}\n"
)
if numbers == numbers2:
...
fixScore = (90 if score >= 90 else
            80 if score >= 80 else score) + 10  # 优先级低于算术运算符
...
obj = {
    ...
    'age': 20,
    'name': '张三',
    'hobbies': ['Swimming', 'Reading', "Listen to music", "Play games"],
    'detail': {
        'height': 175,
        'weight': 70
    }
...
print(f"unknown:{({'from':'China'}).get('unknown','不存在该字段')}\n")
...
obj2 = {
    ...
    'age': 15,
    'name': 'Si Li',
    'hobbies': ['Swimming', 'Reading', "Listen to music", "Play basketboll"],
    'detail': {
        'height': 185,
        'weight': 80
    }
...
print(
    f"from key append China and America, objFromKeysSelf:{objFromKeysSelf}\n")
# 拷贝对象所有字段的数据
...
dictObj = {
    k: obj2.get(k, ['Swimming', 'Reading', 'Coding'])
    for k in ['from', 'age', 'name', 'hobbiesFix']
}
...
setDetailHeight = dictObj.setdefault('detail',
                                     {'height': 177})  # 无键则写入 default 并返回
...
sorted_dictNum2 = sorted(
    dictNum2.items(), key=lambda kv: kv[1],
    reverse=True)  # 按值排序， key=lambda kv: kv[1] 返回元组的第二个元素（即字典的值）
print(f"sorted_dictNum2:{sorted_dictNum2}\n")
sorted_dictNum3 = sorted(
    dictNum2.items(), key=lambdaReplace,
    reverse=False)  # 按值排序， key=lambda kv: kv[1] 返回元组的第二个元素（即字典的值）
print(f"sorted_dictNum3:{sorted_dictNum3}\n")
sorted_dictNum4 = sorted(
    dictNum2.items(), key=itemgetter(1),
    reverse=False)  # 按值排序， key=lambda kv: kv[1] 返回元组的第二个元素（即字典的值）
...
print(
    f"countInitEmpty.update(['banana', 'orange'])， countInitEmpty:{countInitEmpty}\n"
)
# 递减
countInitEmpty.subtract(['banana'])

# Counter({'apple': 1, 'banana': 0, 'orange': 1})
print(
    f"countInitEmpty.subtract(['banana'])， countInitEmpty:{countInitEmpty}\n")
...
print(f"n1:{n1}\n")
n2 = [y for x in numbers
      if (y := x**2) > 10]  # y= 16 , 后续的 y 会共享此 y, [16, 25, 36, 49, 64, 81]
print(f"n2:{n2}\n")
